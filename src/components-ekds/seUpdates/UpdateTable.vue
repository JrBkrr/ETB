  ...
  <div :class="widgetClasses" class="card h-100">
    ...
    <div class="card-header border-0 pt-5">
      <h3 class="card-title align-items-start flex-column">
        <span class="card-label fw-bold fs-3 mb-1">Settings Updates</span>
      </h3>
      ...
      <div class="d-flex align-items-center justify-content-center">
        ...
        <div class="card-toolbar">
          ...
          <button
              ...
              data-bs-toggle="modal" :data-bs-target="`#${'kt_modal_new_update_set'}`"
          >
        ...
        <table class="table align-middle gs-0 gy-4">
          ...
          <tr class="bg-light">
            ...
            <th class="min-w-125px">
              <span class="text-dots text-muted fw-bold mt-1">Oluşturma Tarihi</span>
            </th>
            <th class="min-w-125px">
              <span class="text-dots text-muted fw-bold mt-1">Geçerlilik Tarihi</span>
            </th>
            <th class="min-w-125px">
              <span class="text-dots text-muted fw-bold mt-1">Açıklama Metni</span>
            </th>
          ...
          <!--begin::Table body-->
          <tbody>
          <template v-if="List.length > 0" v-for="(item, index) in List" :key="index">
              ...
              <td>
                <div class="d-flex align-items-center">
                  <div class="d-flex justify-content-start flex-column">
                    <a
                        class="text-dark text-hover-primary fw-bold mb-1 fs-6"
                    >{{ moment(item.effectiveDateTime).format('DD-MM-YYYY hh:mm:ss') }}</a
                    >
                  </div>
                </div>
              </td>
...
export default defineComponent({
  ...
  setup(props, {emit}) {
    ...
    // Get List
    onMounted(() => {
      Action_Start('get', 'settings-updates', 'SeUpdates')
    });
    
    // Filter List
    const SearchText = ref<string>('');
    const List = computed(() => {
      return State.SeUpdates
    });
